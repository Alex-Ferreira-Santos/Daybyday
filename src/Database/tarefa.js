...
export default class TarefaDB{
        ...
        return new Promise(resolve => {
            ...
            SQLite.openDatabase(database_name, database_version, database_display_name, database_size).then(DB =>{
                ...
                db.executeSql('SELECT 1 FROM tarefa LIMIT 1').then(()=>{
                    ...
                    db.transaction(tx => {
                        tx.executeSql('CREATE TABLE IF NOT EXISTS tarefa (id INTEGER PRIMARY KEY AUTOINCREMENT, titulo VARCHAR(255) NOT NULL, descricao VARCHAR(255) NOT NULL, dataDeTermino VARCHAR(255) NOT NULL, prioridade VARCHAR(255) NOT NULL, concluido VARCHAR(255) NOT NULL)')    
                    }).then(()=>{
    ...
    insert(task){
        return new Promise(resolve =>{
            this.initDB().then(db => {
                db.transaction(tx => {
                    tx.executeSql('INSERT INTO tarefa VALUES(?,?,?,?,?,?)',[task.id,task.titulo,task.descricao,task.dataDeTermino,task.prioridade,task.concluido]).then(([tx,results]) => {
                        resolve(results)
                    })
                }).then( results => {
                    this.closeDB(db)
                ...
                db.transaction( tx => {
                    tx.executeSql('SELECT * FROM tarefa',[]).then(([tx,result]) => {
                        ...
                        for(let i = 0; i < len; i++){
                            ...
                            const {id,titulo,descricao,dataDeTermino,prioridade,concluido} = row
                            products.push({id,titulo,descricao,dataDeTermino,prioridade,concluido})
                        }
                        resolve(products)
                    })
    ...
    updateConcluido(concluido,id){
        return new Promise((resolve, reject) =>{
            this.initDB().then( db => {
                db.transaction(tx => {
                    tx.executeSql(`UPDATE tarefa SET concluido = '${concluido}' WHERE id = ${id}`,[]).then(([tx,results])=>{
                        resolve(results)
                    }) 
                }).then((results)=>this.closeDB()).catch(err => console.log(err))
            }).catch(err => console.log(err))
